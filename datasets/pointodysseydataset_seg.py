...
class PointOdysseyDataset(torch.utils.data.Dataset):
    ...
    def getitem_helper(self, index):
        ...
        for mask_path in mask_paths:
            with Image.open(mask_path) as im:
                masks.append(np.array(im)) # H,W,1
        ...
        if self.use_augs and (np.random.rand() < self.spatial_aug_prob):
            rgbs, masks, trajs = self.add_spatial_augs(rgbs, masks, trajs, visibs)
        else:
            # either crop or resize
            if np.random.rand() < 0.5:
                rgbs, masks, trajs = self.just_crop(rgbs, masks, trajs)
            else:
                rgbs, masks, trajs = self.just_resize(rgbs, masks, trajs)
    ...
    def add_spatial_augs(self, rgbs, masks, trajs, visibs):
        T, N, _ = trajs.shape
        ...
        return rgbs, masks, trajs

    def just_crop(self, rgbs, masks, trajs):
        T, N, _ = trajs.shape
    ...
    def just_resize(self, rgbs, masks, trajs):
        T, N, _ = trajs.shape
